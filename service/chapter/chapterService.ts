...
// Cache configuration
const novelCacheMap = new Map<number, { lastFetchTime: number; chapters: Chapter[] }>();
const chapterCacheMap = new Map<string, { lastFetchTime: number; chapter: Chapter }>();
...
const getChapterCacheKey = (novelId: number, chapterId: number) => `chapter_${novelId}_${chapterId}`;

// Obtener todos los capítulos de una novela
export const getChaptersAsync = (novelId: number) => async (dispatch: AppDispatch): Promise<Chapter[] | void> => {
  const now = Date.now();
      ...
      try {
        ...
        if (error instanceof AxiosError) {
          ...
          switch (status) {
            case 401:
              ...
              dispatch(setError("No autorizado para acceder a los capítulos"));
              throw error;
        ...
        if (attempt === MAX_RETRIES - 1) {
          const errorMessage = error instanceof AxiosError 
            ? `Error fetching chapters (${error.response?.status}): ${error.response?.data?.message || error.message}`
...
export const getChapterAsync = (novelId: number, chapterId: number) => async (dispatch: AppDispatch): Promise<Chapter | void> => {
  ...
  const fetchRequest = async (): Promise<Chapter | void> => {
    ...
    for (let attempt = 0; attempt < MAX_RETRIES; attempt++) {
      try {
        ...
        if (attempt === MAX_RETRIES - 1) {
          const errorMessage = error instanceof AxiosError 
            ? `Error fetching chapter (${error.response?.status}): ${error.response?.data?.message || error.message}`
...
export const getChapterCacheInfo = () => {
  return {
    ...
    novelCacheEntries: Array.from(novelCacheMap.entries()).map(([novelId, cache]) => ({
      novelId,
    ...
    chapterCacheEntries: Array.from(chapterCacheMap.entries()).map(([key, cache]) => ({
      key,
      cacheAge: Date.now() - cache.lastFetchTime,
      isExpired: Date.now() - cache.lastFetchTime > CACHE_DURATION
    }))
