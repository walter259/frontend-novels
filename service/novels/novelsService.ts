...
// Cache configuration
const novelCacheMap = new Map<string, { lastFetchTime: number; novel: Novel }>();
const novelsListCacheMap = new Map<string, { lastFetchTime: number; novels: Novel[] }>();
...
export const getNovelsAsync = () => async (dispatch: AppDispatch): Promise<Novel[] | void> => {
      ...
      try {
        ...
        if (attempt === MAX_RETRIES - 1) {
          const errorMessage = error instanceof AxiosError 
            ? `Error fetching novels (${error.response?.status}): ${error.response?.data?.message || error.message}`
...
export const getNovelByIdAsync = (id: string) => async (dispatch: AppDispatch): Promise<Novel | void> => {
  const now = Date.now();
  ...
  const fetchRequest = async (): Promise<Novel | void> => {
    ...
    for (let attempt = 0; attempt < MAX_RETRIES; attempt++) {
      try {
        ...
        if (attempt === MAX_RETRIES - 1) {
          const errorMessage = error instanceof AxiosError 
            ? `Error fetching novel (${error.response?.status}): ${error.response?.data?.message || error.message}`
...
export const getNovelsCacheInfo = () => {
  return {
    ...
    novelCacheEntries: Array.from(novelCacheMap.entries()).map(([id, cache]) => ({
      id,
      cacheAge: Date.now() - cache.lastFetchTime,
      isExpired: Date.now() - cache.lastFetchTime > CACHE_DURATION
    })),
    novelsListCacheEntries: Array.from(novelsListCacheMap.entries()).map(([key, cache]) => ({
      key,
